...
int main() {
	...
	root->autoFlag = root->totalTime = 0;
...
int modified_recommend(RecNode* root) {
	...
	root->level = root->accScore = 0;
	root->x = root->y = root->r = 0;
	root->super = NULL;
	...
	modified_recurRec(root, &max, &xRec, &yRec, &rRec, &nodeCnt);
	recommendX = xRec;
	recommendY = yRec;
	recommendR = rRec;
...
int findNumChild(RecNode* Troot) {

	int rotate, xCoor, yTop, yTemp, yHeight, numChild;
	yTop = 0; numChild = 1;
	...
	for (rotate = 0; rotate < 4; rotate++) {
		for (xCoor = -3; xCoor < WIDTH + 3; xCoor++) {
			...
			yHeight = findMaxHeight(nextBlock[Troot->level], rotate, yTemp);

			if (yTop > yHeight) continue;
...
int findMaxHeight(int currentBlock, int rotate, int yField) {

	int xCoor, yCoor;

	for (yCoor = 0; yCoor < BLOCK_HEIGHT; yCoor++)
		for (xCoor = 0; xCoor < BLOCK_WIDTH; xCoor++)
			if (block[currentBlock][rotate][yCoor][xCoor])
				return (yCoor + yField);

	return yField;
}

void modified_recurRec(RecNode* Troot, int* maxScore, int* xR, int* yR, int* rR, int* nodeCnt) {

   	(*nodeCnt)++;
	if (Troot->level >= VISIBLE_BLOCKS) return;

	char tempField[HEIGHT][WIDTH];
	int i, rotate, xCoor, yTop, yTemp, yHeight, cIdx, numChild, tempScore;
	yTop = cIdx = 0;

	numChild = findNumChild(Troot);
	Troot->arr = (RecNode**)malloc(sizeof(RecNode*) * numChild);

	for (rotate = 0; rotate < 4; rotate++) {
		for (xCoor = -4; xCoor < WIDTH + 3; xCoor++) {
			...
			if (cIdx > 0 && yTop < yHeight) cIdx = 0;

			yTop = yHeight;

			copyField(Troot->recField, tempField);

			tempScore = Troot->accScore;
			tempScore += AddBlockToField(tempField, nextBlock[Troot->level], rotate, yTemp, xCoor);
			tempScore += checkTouch(tempField, nextBlock[Troot->level], rotate, yTemp, xCoor);
			tempScore += DeleteLine(tempField);

		    if (cIdx > 0 && tempScore < Troot->arr[cIdx - 1]->accScore) continue;

			Troot->arr[cIdx] = (RecNode*)malloc(sizeof(RecNode));

			copyField(tempField, Troot->arr[cIdx]->recField);
			Troot->arr[cIdx]->accScore = tempScore;
	...
	for (i = 0; i < cIdx; i++) {
		...
		modified_recurRec(Troot->arr[i], maxScore, xR, yR, rR, nodeCnt);
...
int checkTouch(char f[HEIGHT][WIDTH], int currentBlock, int rotate, int blockY, int blockX) {
	int yCoor, xCoor, count, touchScore;
	count = touchScore = 0;

	for (yCoor = 0; yCoor < BLOCK_HEIGHT; yCoor++) {
		for (xCoor = 0; xCoor < BLOCK_WIDTH; xCoor++)
			if (block[currentBlock][rotate][yCoor][xCoor] && (yCoor + blockY) < HEIGHT && (xCoor + blockX) < WIDTH) {

				if (yCoor + blockY + 1 < HEIGHT && !block[currentBlock][blockRotate][yCoor + 1][xCoor] && !f[blockY + yCoor + 1][blockX + xCoor]) touchScore -= 4;
				if (f[blockY + yCoor][blockX + xCoor + 1]) touchScore += 2;
				if (f[blockY + yCoor][blockX + xCoor - 1]) touchScore += 2;
