...
void DrawChange(char f[HEIGHT][WIDTH],int command,int currentBlock,int blockRotate, int blockY, int blockX){
    ...
    // ProcessCommand로부터 CheckToMove가 된 값을 받는다. 즉 인자값들은 한 단계 이후의 블록 변수들

    int yCoor, xCoor, delX, delY, delRt, count = 0;

    delX = blockX; delY = blockY; delRt = blockRotate;
...
void AddBlockToField(char f[HEIGHT][WIDTH],int currentBlock,int blockRotate, int blockY, int blockX){
    ...
    int yCoor, xCoor, count = 0;

    for (yCoor = 0; yCoor < BLOCK_HEIGHT; yCoor++){
        for (xCoor = 0; xCoor < BLOCK_WIDTH; xCoor++)
            if(block[currentBlock][blockRotate][yCoor][xCoor]) 
                if ((blockY + yCoor) < HEIGHT && (blockX + xCoor) < WIDTH) {
...
int DeleteLine(char f[HEIGHT][WIDTH]){
	...
	int xCoor, yCoor, tmpY, countScore = 0;
    ...
    DrawField();
    return countScore * countScore * 100;
}
